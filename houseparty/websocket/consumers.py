...
class WebSocketConsumer(AsyncWebsocketConsumer):
    ...
    async def set_current_song(self, event):
        song = event['song']
        action = event['action']

        await self.set_currently_playing_song(self.current_room, song)
    ...
    @database_sync_to_async
    def set_currently_playing_song(self, code, song):
        try:
            room = Room.objects.select_related('host__user', 'player', 'listener').prefetch_related('player__skip_votes__user', 'listener__active_users__user').get(code=code)
            print('current song:', song)
            room.player.current_song = song
            room.player.save()
        except Exception as e:
            print(e)